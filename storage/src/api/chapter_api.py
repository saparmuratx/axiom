...
@chapter_router.post(
    "/chapter", response_model=ChapterCreateSchema, status_code=status.HTTP_201_CREATED
)
async def create_chapter(
    ...
    try:
        chapter = await service.create_chapter(data)

        service.repository.session.refresh(chapter.object)

        logger.info(chapter.object.to_dict())
...
@chapter_router.get("/chapter", response_model=list[ChapterSchema])
...
async def update_chapter(
    ...
    try:
        return await service.update_chapter(str(chapter_id), data)
    except NotFoundException:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Chapter not found")
    except Exception as e:
        raise HTTPException(status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e))
